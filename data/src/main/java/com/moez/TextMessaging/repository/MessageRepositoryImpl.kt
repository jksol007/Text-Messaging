...
import com.text.messages.sms.emoji.common.util.extensions.now
...
import com.text.messages.sms.emoji.manager.KeyManager
...
import com.text.messages.sms.emoji.receiver.SendSmsReceiver
import com.text.messages.sms.emoji.receiver.SmsDeliveredReceiver
import com.text.messages.sms.emoji.receiver.SmsSentReceiver
import com.text.messages.sms.emoji.util.ImageUtils
...
class MessageRepositoryImpl @Inject constructor(
    ...
    override fun updateIsEmoji(id: Long, emojiOnly: Boolean) {
        val realm = Realm.getDefaultInstance()
        realm.executeTransactionAsync {
            val messages = it.where(Message::class.java)
                .equalTo("id", id)
                .findFirst()

            messages?.emojiOnly = if (emojiOnly) 1 else 0
        }
        realm.close()
    }
...
//                            sent = false
//                        }
                        //  logDebug("delay" + x + ">>>" + addresses.size)
                    }
                } else {
            ...
            val imageBytesByAttachment = attachments
                ...
                .associateWith { attachment ->
                    ...
                    when (attachment.isGif(context)) {
                        ...
                        false -> getBytes(uri.path)
                        // false -> ImageUtils.getScaledImage(context, uri, maxWidth, maxHeight)
                        //  false -> ImageUtils.getScaledImage(context, this!!.scaleToSend(context, attachment.getUri()!!, ContentType.IMAGE_JPEG, maxWidth, maxHeight)!!, maxWidth, maxHeight)
                    }
                }
                .toMutableMap()
...
//                            false -> ImageUtils.getScaledImage(context, uri, newWidth, newHeight, 80)
    ...
    private fun sendSms(message: Message, addresses: List<String>) {
        ...
        val sentIntents = parts.map {
            ...
            PendingIntent.getBroadcast(
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) PendingIntent.FLAG_MUTABLE or
                        PendingIntent.FLAG_UPDATE_CURRENT else PendingIntent.FLAG_UPDATE_CURRENT
            )
        }
    ...
    private fun sendSms(message: Message, addresses: List<String>, x: Int) {
        val smsManager = message.subId.takeIf { it != -1 }
            ?.let(SmsManagerFactory::createSmsManager)
            ?: SmsManager.getDefault()
