...
import com.text.messages.sms.emoji.extensions.anyOf
import com.text.messages.sms.emoji.extensions.asObservable
import com.text.messages.sms.emoji.extensions.map
import com.text.messages.sms.emoji.extensions.removeAccents
import com.text.messages.sms.emoji.filter.ConversationFilter
import com.text.messages.sms.emoji.manager.PermissionManager
import com.text.messages.sms.emoji.mapper.CursorToConversation
import com.text.messages.sms.emoji.mapper.CursorToRecipient
...
class ConversationRepositoryImpl @Inject constructor(
    ...
    override fun searchConversationsByNumber(): RealmResults<Conversation> {
        return Realm.getDefaultInstance()
            .where(Conversation::class.java)
            .notEqualTo("id", 0L)
            .findAllAsync()
    ...
    override fun getConversations(vararg threadIds: Long): RealmResults<Conversation> {
        ...
        var conversation: RealmResults<Conversation> =
            Realm.getDefaultInstance().where(Conversation::class.java).anyOf("id", threadIds)
                .findAll()
    ...
    override fun getThreadId(recipient: String): Long? {
        return getThreadId(listOf(recipient),true)
    }

    override fun getThreadId(recipients: Collection<String>, isRefreshNeed: Boolean): Long? {
        return Realm.getDefaultInstance().use { realm ->
            if (isRefreshNeed) {
                realm.refresh()
            }
    ...
    override fun getOrCreateConversation(threadId: Long): Conversation? {
        return getConversation(threadId) ?: getConversationFromCp(threadId, false)
    }

    override fun getOrCreateConversation(address: String): Conversation? {
        return getOrCreateConversation(listOf(address), isAfterCall = true, isRefreshNeed = true)
    }

    override fun getOrCreateConversation(addresses: List<String>): Conversation? {
        return getOrCreateConversation(addresses, isAfterCall = false, isRefreshNeed = true)
    }

    override fun getOrCreateConversationWithoutRefresh(addresses: List<String>): Conversation? {
        return getOrCreateConversation(addresses, isAfterCall = false, isRefreshNeed = false)
    }

    /*override fun getOrCreateConversation(addresses: List<String>): Conversation? {
        if (addresses.isEmpty() || !permissionManager.hasReadSms()) {
            return null
        }
    ...
    private fun getOrCreateConversation(
        ...
        return (getThreadId(addresses, isRefreshNeed) ?: tryOrNull {
            TelephonyCompat.getOrCreateThreadId(
                context,
                addresses.toSet()
            )
            ...
            ?.let { threadId ->
                getConversation(threadId)
                    ?.let(Realm.getDefaultInstance()::copyFromRealm)
                    ?: getConversationFromCp(threadId, isAfterCall)
            }
    }
     ...
     */
    private fun getConversationFromCp(threadId: Long, isAfterCall: Boolean): Conversation? {
        return cursorToConversation.getConversationsCursor()
            ...
            ?.let { conversation ->
                ...
                if (isAfterCall) {
                    realm.executeTransactionAsync { it.insertOrUpdate(conversation) }
                } else {
                    realm.executeTransaction { it.insertOrUpdate(conversation) }
                }
                realm.close()
