...
import com.text.messages.sms.emoji.mapper.CursorToPartImpl
import com.text.messages.sms.emoji.receiver.*
...
import com.text.messages.sms.emoji.util.GlideApp
...
class NotificationManagerImpl @Inject constructor(
...
) : com.text.messages.sms.emoji.manager.NotificationManager {
    ...
    override fun update(threadId: Long) {
        ...
        val contentPI = taskStackBuilder.getPendingIntent(threadId.toInt(), if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) PendingIntent.FLAG_MUTABLE or
        ...
        val notification = NotificationCompat.Builder(context, getChannelIdForNotification(threadId))
                ...
                .setColor(context.resources.getColor(R.color.tools_theme))
                    ...
                    when (action) {
                        Preferences.NOTIFICATION_ACTION_ARCHIVE -> {
                            ...
                            val pi = PendingIntent.getBroadcast(context, threadId.toInt(), intent,
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) PendingIntent.FLAG_MUTABLE or PendingIntent.FLAG_UPDATE_CURRENT else PendingIntent.FLAG_UPDATE_CURRENT)
                            NotificationCompat.Action.Builder(R.drawable.ic_arch_vector, actionLabels[action], pi)
                        ...
                        Preferences.NOTIFICATION_ACTION_BLOCK -> {
                            ...
                            val pi = PendingIntent.getBroadcast(context, threadId.toInt(), intent,
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) PendingIntent.FLAG_MUTABLE or PendingIntent.FLAG_UPDATE_CURRENT else PendingIntent.FLAG_UPDATE_CURRENT)
                            NotificationCompat.Action.Builder(R.drawable.ic_action_block, actionLabels[action], pi)
                        ...
                        Preferences.NOTIFICATION_ACTION_REPLY -> {
                            if (Build.VERSION.SDK_INT >= 24) {
                                ...
                                val intent = Intent(context, MessagesReplyActivity::class.java).putExtra("threadId", threadId)
        ...
        if (prefs.qkreply.get()) {
            ...
            val intent = Intent(context, MessagesReplyActivity::class.java)
                    .putExtra("threadId", threadId)
