...
import com.text.messages.sms.emoji.common.util.TextViewStyler.Companion.SIZE_PRIMARY
import com.text.messages.sms.emoji.common.util.TextViewStyler.Companion.SIZE_SECONDARY
import com.text.messages.sms.emoji.common.util.TextViewStyler.Companion.SIZE_TERTIARY
import com.text.messages.sms.emoji.common.util.TextViewStyler.Companion.SIZE_TOOLBAR
import com.text.messages.sms.emoji.common.util.extensions.getColorCompat
...
class TextViewStyler @Inject constructor(
        ...
        fun applyEditModeAttributes(textView: TextView, attrs: AttributeSet?) {
            textView.run {
                ...
                when (this) {
                    ...
                    is MessagesEditText -> context.obtainStyledAttributes(attrs, R.styleable.QkEditText)
                        ?.run {
    ...
    fun applyAttributes(textView: TextView, attrs: AttributeSet?) {
        ...
        when (textView) {
            is MessagesTextView -> textView.context.obtainStyledAttributes(attrs, R.styleable.QkTextView)
                .run {
                    colorAttr = getInt(R.styleable.QkTextView_textColor, -1)
                    textSizeAttr = getInt(R.styleable.QkTextView_textSize, -1)
                    recycle()
            ...
            is MessagesEditText -> textView.context.obtainStyledAttributes(attrs, R.styleable.QkEditText)
                .run {
                    colorAttr = getInt(R.styleable.QkEditText_textColor, -1)
                    textSizeAttr = getInt(R.styleable.QkEditText_textSize, -1)
                    recycle()
        ...
        if (textView is EditText) {
            ...
            val drawable = textView.resources.getDrawable(R.drawable.cursor)
                .apply { setTint(textView.resources.getColor(R.color.tools_theme)) }
