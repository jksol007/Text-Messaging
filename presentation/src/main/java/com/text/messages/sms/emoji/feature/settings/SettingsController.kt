...
import com.text.messages.sms.emoji.common.MenuItem
...
import com.text.messages.sms.emoji.common.util.extensions.animateLayoutChanges
...
import com.text.messages.sms.emoji.feature.settings.autodelete.AutoDeleteDialog
...
class SettingsController : MessagesController<SettingsView, SettingsState, SettingsPresenter>(),
    SettingsView {
    ...
    @Inject
    lateinit var nightModeDialog: MessagesDialog

    @Inject
    lateinit var textSizeDialog: MessagesDialog

    @Inject
    lateinit var sendDelayDialog: MessagesDialog

    @Inject
    lateinit var mmsSizeDialog: MessagesDialog
    ...
    init {
        ...
        colors.themeObservable().autoDispose(scope()).subscribe { activity?.recreate() }
    }
        ...
        when (Build.VERSION.SDK_INT >= 29) {
            ...
            false -> nightModeDialog.adapter.data =
                context.resources.getStringArray(R.array.night_modes)
                    .mapIndexed { index, title -> MenuItem(title, index) }.drop(1)
        }
...
//    }
    ...
    override fun preferenceClicks(): Observable<PreferenceView> = (0 until preferences.childCount)
        .map { index -> preferences.getChildAt(index) }
        .mapNotNull { view -> view as? ViewGroup }
        .map { preference ->
            (0 until preference.childCount).map { i -> preference.getChildAt(i) }
                .mapNotNull { view -> view as? PreferenceView }
                .map { preference1 -> preference1.clicks().map { preference1 } }
                .let { preferences -> Observable.merge(preferences) }
        }
        .let { preferences -> Observable.merge(preferences) }
    ...
    override fun render(state: SettingsState) {
        ...
        themePreview.setTint(resources!!.getColor(R.color.tools_theme), true)
        //themePreview.setBackgroundTint(resources!!.getColor(R.color.tools_theme))
...
//        black.checkbox.isChecked = state.black
        ...
        Log.i("checkbox", state.systemFontEnabled.toString())
        ...
        when (state.syncProgress) {
            ...
            is SyncRepository.SyncProgress.Running -> {
                ...
                progressAnimator.apply {
                    setIntValues(
                        syncingProgress.progress, state.syncProgress.progress
                    )
                }.start()
    ...
    override fun showStartTimePicker(hour: Int, minute: Int) {
        Log.i("TAG", "showStartTimePicker : ")
        TimePickerDialog(activity, R.style.AppPickerDialog, { _, newHour, newMinute ->
    ...
    override suspend fun showAutoDeleteWarningDialog(messages: Int): Boolean =
        withContext(Dispatchers.Main) {
            suspendCancellableCoroutine<Boolean> { cont ->
                ...
                ).setTitle(R.string.settings_auto_delete_warning).setMessage(
                    context.resources.getString(
                        R.string.settings_auto_delete_warning_message, messages
                    )
                ).setOnCancelListener { cont.resume(false) }
                    .setPositiveButton(R.string.button_yes) { _, _ -> cont.resume(true) }
                    .setNegativeButton(R.string.button_cancel) { _, _ -> cont.resume(false) }.show()
            }
        }
    ...
    override fun showSwipeActions() {
        router.pushController(
            RouterTransaction.with(SwipeActionsController()).pushChangeHandler(MessagesChangeHandler())
    ...
    override fun showThemePicker() {
        router.pushController(
            RouterTransaction.with(ThemePickerController()).pushChangeHandler(MessagesChangeHandler())
                .popChangeHandler(MessagesChangeHandler())
        )
    }
