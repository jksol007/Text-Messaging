...
import com.text.messages.sms.emoji.filter.ContactFilter
import com.text.messages.sms.emoji.filter.ContactGroupFilter
import com.text.messages.sms.emoji.interactor.SetDefaultPhoneNumber
...
class ContactsViewModel @Inject constructor(
...
) : MessagesViewModel<ContactsContract, ContactsState>(ContactsState(isKeyboard = true)) {
    ...
    fun onCLick(query: CharSequence, view: ContactsContract, isKeyboard: Boolean) {
        ...
        Observables
            .combineLatest(
                ...
                contacts,
                selectedChips
            ) {  /*recents, starredContacts, contactGroups,*/ contacts, selectedChips ->
                val composeItems = mutableListOf<ComposeItem>()
                if (query.isBlank()) {
//                    composeItems += recents
//                        .filter { conversation ->
